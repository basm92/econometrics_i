...
```{r}
Xhat[3] # what is the third entry? Ask!
```
...
```{r}
model1 <- coeftest(lm(d_sh_empl_mfg ~ d_tradeotch_pw_lag, data=subset(chinapreperiod, yr==1990)), vcov = vcovCL, cluster = ~statefip)
model2 <- coeftest(lm(d_sh_empl_mfg ~ d_tradeotch_pw_lag, data=subset(chinapreperiod, yr==2000)), vcov = vcovCL, cluster = ~statefip)
model3 <- coeftest(lm(d_sh_empl_mfg ~ d_tradeotch_pw_lag + t1990 + t2000, data=subset(chinapreperiod, yr==1990|yr==2000)), vcov = vcovCL, cluster = ~statefip)
model4 <- coeftest(lm(d_sh_empl_mfg ~ d_tradeotch_pw_lag_future, data=subset(chinapreperiod, yr==1970)), vcov = vcovCL, cluster = ~statefip)
model5 <- coeftest(lm(d_sh_empl_mfg ~ d_tradeotch_pw_lag_future, data=subset(chinapreperiod, yr==1980)), vcov = vcovCL, cluster = ~statefip)
model6 <- coeftest(lm(d_sh_empl_mfg ~ d_tradeotch_pw_lag_future + t1970 + t1980, data=subset(chinapreperiod, yr==1970|yr==1980)), vcov = vcovCL, cluster = ~statefip)
```

Could not reproduce the right numbers yet!
