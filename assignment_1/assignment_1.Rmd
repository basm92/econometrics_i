...
$$
\mathbb{E}[\alpha_{OMV}] = \mathbb{E}[(D^T D)^{-1}D^T y] = 
$$
...
$$
\text{Var}(\alpha_{OLS}) = \sigma^2 (X^TX)^{-1} 
$$

Then, using the result by Heij on p. 144 that the covariance between the estimators is zero, we can evaluate the difference between the variance as:

$$
\text{Var}(\hat{\alpha}_{OLS}) - \text{Var}(\hat{\alpha}_{OMV}) = \\
\text{Var}(\hat{\alpha}_{OLS} - \hat{\alpha}_{OMV}) = 
$$

$$
\text{Var}(\hat{\alpha}_{OLS})  + \text{Var}(\hat{\alpha}_{OMV}) = \\
\sigma^2 (X^TX)^{-1} + \sigma^2(D^TD)^{-1} \text{ where } X = [D, O, C]
$$
Now, because of the positive variance property of all independent variables, we know that $(X^TX)^{-1}$ and $(D^TD)^{-1}$ are positive semidefinite matrices, so that the difference in variances between the OLS estimator and the OMV estimator is strictly positive. 
