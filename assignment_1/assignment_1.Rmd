...
Wages is an outcome variable, which consist partially of hours people have work (at a paid job). Hours spent on childcare is als another way you can use your time, hence, is also an outcome variable. Hence, part of the effect of gender on wages is happening through the effect of working on childcare. Hence, conditioning on childcare would imply 'conditioning on the outcome', and thus downwardly biasing your coefficient of interest (i.e. gender). Therefore, it would generally not be a good idea. 
...
d. The researcher further assumes that $\beta$ is known but $lamda$ is unknown. Derive $\hat\lamda$, the maximum likelihood estimator for $\lamda$. You do not need to verify
the second-order condition in this question.\\

Taking the first derivative of the log-likelihood with respect to $\lamda$ and setting it to zero, gives us:

$$
-\frac{n}{\lambda} + \beta \sum_{i=1}^N \left(\frac{x_i}{\lambda}^(\beta + 1)\right) - n(\beta-1) = 0
$$

Reaaranging for $\lamda$, gives us the following maximum likelihood estimate for $\lamda$:

$$
\hat{\lamda} = \sum_{i=1}^N \left(\frac{x_i}{\sqrt[\beta]{n}}\right)
$$
...
(Intercept) & \begin{tabular}[c]{@{}l@{}}8.1336310***\\ (0.0297187)\end{tabular}  & 8.133631029  \\
age         & \begin{tabular}[c]{@{}l@{}}-0.0013215\\ (0.0008445)\end{tabular}    & -0.001321462 \\
smoker      & \begin{tabular}[c]{@{}l@{}}-0.0563769***\\ (0.0102227)\end{tabular} & -0.056376933 \\
alcohol     & \begin{tabular}[c]{@{}l@{}}-0.0130960\\ (0.0352059)\end{tabular}    & -0.013095958 \\
drinks      & \begin{tabular}[c]{@{}l@{}}-0.0020363\\ (0.0070214)\end{tabular}    & -0.002036251 \\
unmarried   & \begin{tabular}[c]{@{}l@{}}-0.0881805***\\ (0.0104432)\end{tabular} & -0.088180499 \\
educ        & \begin{tabular}[c]{@{}l@{}}0.0031101\\ (0.0020467)\end{tabular}     & 0.003110116 
\end{tabular}
\end{table}
